...
void vLIS2_EnableFifoSampling(void) {
  ...
  vTWI_Write(CTRL_REG1, (ODR_1620Hz | X_EN | Y_EN | Z_EN));
  //ODR_1620Hz
  //ODR_10Hz
  ...
  vTWI_Write(CTRL_REG4, (HIRES_MODE | FS_16G | BDU_ENABLE));
...
void dumpData(uint8_t u8Num) {
  uint8_t u8Cnt;
  int16_t x_mean=0, y_mean=0, z_mean=0,x_mean1=0, y_mean1=0, z_mean1=0;
  /*
  for (u8Cnt = 0; u8Cnt < u8Num; u8Cnt++) {
    // Convert twos-compliment into signed 16-bit integer and remove left justification
    //printf("%d %d %d\n",
  ...
  x_mean = ((int16_t)atsFifo[u8Cnt].u16_X) / (1 << u8SignExtend);
  y_mean = ((int16_t)atsFifo[u8Cnt].u16_Y) / (1 << u8SignExtend);
  z_mean = ((int16_t)atsFifo[u8Cnt].u16_Z) / (1 << u8SignExtend);

  for (u8Cnt = 1; u8Cnt < u8Num; u8Cnt++) {
    x_mean1 = ((int16_t)atsFifo[u8Cnt].u16_X) / (1 << u8SignExtend);
    y_mean1 = ((int16_t)atsFifo[u8Cnt].u16_Y) / (1 << u8SignExtend);
    z_mean1 = ((int16_t)atsFifo[u8Cnt].u16_Z) / (1 << u8SignExtend);

    x_mean = (x_mean + x_mean1) / 2;
    y_mean = (y_mean + y_mean1) / 2;
    z_mean = (z_mean + z_mean1) / 2;
  }

  printf(",%d,%d,%d",x_mean, y_mean, z_mean);
}